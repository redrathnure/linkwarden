...
3. Based on latest [Debian Trixie slim](https://hub.docker.com/_/node) images
4. Experimental build with node.js 24 (✓)
5. Experimental build with node.js 26 (✓)
6. Images were optimized for Docker BuildX toolset (reorder docker layers, use cache mount points etc)
...
services:
  ...
  linkwarden:
    image: ghcr.io/redrathnure/linkwarden:latest
    # or from Docker hub
    #image: docker.io/redrathnure/linkwarden:latest
    env_file: .env
    environment:
...
Node.js 22 (Debian Trixie based):
...
Node.js 24 (Debian Trixie based): same as nodejs 20 but with `-node24` suffix.
Node.js 26 (Debian Trixie based): same as nodejs 20 but with `-node26` suffix.

**Warning** node22 images are not supported starting from 2.15.x. Please switch to node24 one.
